...
export default function Fragrances({ fragrances }) {
  return (
    <section className="mb-16 px-4 sm:px-6 lg:px-8">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-10">
        <div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-800 mb-2 leading-tight">
            Fresh Fragrances
          </h2>
          <p className="text-gray-600 text-base sm:text-lg">
            Discover our premium selection of fragrances
          </p>
        </div>
        ...
        <Link
          ...
          style={{
            backgroundImage: "url('/assets/web.webp')",
      ...
      {fragrances && fragrances.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6 md:gap-8 mb-12">
          {fragrances.slice(0, 5).map((item) => (
            <ProductCard
              key={item.id}
